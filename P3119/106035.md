### 感觉题解中没有什么用分层图最短路的解法，那我就来分享一下我的做法吧。首先这道题要缩点，应该是扫一眼样例就能看出来的。如果缩点不会请出门左转强连通分量的各种例题，这道题的难点就在如何处理那一次逆行，这个时候我想到了分层图最短路（这个不会的话可以出门P4568飞行线路）我可以用dis[i][use_time]记录到i点并用了use_time次逆行的最大经过点数。接下来还是在代码中解释吧
## （4.17k代码预警）
```
#include<bits/stdc++.h>
#include<stack>
