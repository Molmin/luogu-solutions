> [CF963E Circles of Waiting](https://www.luogu.com.cn/problem/CF963E)

设 $f_{i, j}$ 表示从 $(i, j)$ 走到圆外的期望时间。显然，当 $i ^ 2 + j ^ 2 > R ^ 2$ 时，$f_{i, j} = 0$，否则
$$
f_{i, j} = p_1f_{i - 1, j} + p_2 f_{i, j - 1} + p_3 f_{i + 1, j} + p_4 f_{i, j + 1} + 1
$$
高斯消元 $R ^ 6$ 不可接受。考虑主元法，令 $(i, j)$ 在圆内且 $(i, j + 1)$ 在圆外的 $f_{i, j}$ 为主元，每个 $f_{i, j}$ 提供用 $f_{i - 1, j}$，$f_{i + 1, j}$ 和 $f_{i, j + 1}$ 表示 $f_{i, j - 1}$ 的方式，从上往下推得用每个主元表示 $f_{i, j}$ 的方式，其中 $(i, j)$ 在圆内且 $(i, j - 1)$ 在圆外，然后再利用 $f_{i, j}$ 本身的方程列出 $2R + 1$ 个 $2R + 1$ 元方程。高斯消元求解即可。时间复杂度 $\mathcal{O}(R ^ 3)$。
