...
题目要求：$\sum\limits_{x=1}^N\sigma_0(x^2)$。

一些前置结论：

1. 令 $F(x)=\sigma_0(x^2)$。$\sigma_0(x)$ 是积性函数，从而 $F(x)$ 是两个积性函数的复合，也是积性函数。
2. 令 $p\in prime$，则有 $\sigma_0(p^k)=k+1,F(p^k)=2k+1$，于是有 $\sum\limits_{x=2}^n[x\in prime]F(x)=3\pi(n)$。

3. 上文中，我们已经学会了如何筛出所有 $\pi(\dfrac N x)$。

定义函数 $S(n,k)=\sum\limits_{x=1}^n[x\in prime\lor minp(x)>p_k]F(x)$，则有 $S(n,\pi(\sqrt n)+k)=\sum\limits_{x=2}^n[x\in prime]F(x)=3\pi(n),k\ge0$。
...
int main(){
	...
	while(T--){
		...
		for(i=1;i<=m;++i)f[i]*=3;//2k+1，这里把质数部分求出来了
		for(i=1;i<=pt;++i)sm[i]=sm[i-1]+3; 
		for(x=pt;x;--x)
			for(y=1;pm[x]*pm[x]<=b[y]&&y<=m;++y)
				for(nw=pm[x],l=1;nw*pm[x]<=b[y];nw*=pm[x],++l)
					f[y]+=(l+l+1)*(f[Id(b[y]/nw)]-sm[x])+(l+l+3);就是上面的递推式
		printf("%llu\n",f[1]+1);别忘了加F(1)
	}return 0;
}
```