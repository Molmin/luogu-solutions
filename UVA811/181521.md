...
给 $n$ 个点 $(n<=15)$ ，每个点有一个长度值和价值。现在要删掉若干点，用它们的长度值将剩余的点围起来。

为使围栏周长最小，可知要使用凸包。若凸包的周长小于删掉点的长度值则可以这么砍。最后统计砍掉的树的价值，找到价值最小的方案，若价值一样则寻找剩余木材最小的情况。
...
double solve(int len,int fn){
	...
	for(int i=1;i<=n;i++){
		if(node[i].y>node[sn].y)sn=i;
		else if(node[i].y==node[sn].y&&node[i].x>node[sn].x)sn=i;
	}
	node[0]=node[sn];
	for(int i=1;i<=n;i++){
	...
	node[0].k=1e9;
	for(int i=1;i<=n;i++){
//		if(node[i].k==node[i-1].k&&node[i].k!=sp)continue;
