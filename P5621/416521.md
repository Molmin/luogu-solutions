...
int n,cl,c[50010],st[50010],ed[50010],que[3][50010],en[3],tot;
int xx,yy,zz,cc[3][200010],p[3];
long long ans[50010],mp[100][100][100],b[200010];
struct poin
{
	int a,b,c,d;
	long long v;
...
inline long long find(register int x,register int y,register int z)
{
	long long an=0;
...
int main()
{
	n=qread();
	cl=pow(n,0.333333)+1;
	for(register int i=1;i<=n;++i)b[++tot]=a[i].a=qread(),b[++tot]=a[i].b=qread(),b[++tot]=a[i].c=qread(),b[++tot]=a[i].d=qread(),a[i].v=qread(),c[i]=(i-1)/cl+1,ed[c[i]]=i;
	sort(b+1,b+1+tot);
	...
	for(register int i=1;i<=c[n];++i)
	{
		...
		for(register int j=st[i];j<=ed[i];++j)
		{
			...
			yy=cc[1][a[j].c];
			zz=cc[2][a[j].d];
			ans[j]=mp[xx][yy][zz]+a[j].v;
			for(register int k=st[i];k<j;++k)if(check(k,j))ans[j]=max(ans[j],ans[k]+a[j].v);
		}
