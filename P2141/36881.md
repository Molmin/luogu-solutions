# 2019/7/26更新的新内容（不含原内容）
### PS：时隔2年发现这篇题解居然这么受欢迎，但是看看2年前的代码，感觉好丑。。。所以更新一下并解释一下评论中的几个问题（顺便简化一下）

### 解释:
### 1.本题if (t[i]>0&&g[i])表示i这个数被加出来且不在原集合中
### 2.如果有一个数被多次加出，则在t[i]中多次叠加，但不改变答案。
### 3.ans在主函数外定义，故初值为0
### 4.其他修改见代码
代码：
```cpp
...
## 但是注意：一定要开一个标记保存当前数是否在读入的数中，详细看代码。
