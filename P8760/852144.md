...
## 问题 A，思路：
其实这题只需要把 $11111111$ 从二进制转化成十进制就行了，进制转换代码如下：
```cpp
int main(){
	string s = "11111111";
	int quanzhi = 1, ans = 0;//quanzhi就是权值的拼音
	for(int i = 7; i >= 0; i--){//二进制转十进制
		ans += quanzhi * (s[i] - '0');//按权展开再求和
		quanzhi *= 2;//权值扩大到原来的两倍
	}
	cout << ans;
}
```
...
int main(){
	...
	if(c == 'A') cout << 255;//问题A 
	if(c == 'B') cout << 25;//问题B
	if(c == 'C') cout << 1903;//问题C
	if(c == 'D') cout << 977;//问题D
	if(c == 'E') cout << 2653631372;//问题E
} 
```