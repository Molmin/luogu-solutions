...
块内排序一下，在查询操作进行二分，从而找到块内比 $k$ 大的值的个数。
```cpp
void build(){
	block=sqrt(n);
	tot=n/block;
...
对于查询操作，散块直接暴力查询即可。整块则在块内通过二分得到答案。

我们在预处理的时候，把每个块都进行了排序。
...
int query(int l,int r,int k){
	...
	if(pos[l]==pos[r]){
		...
		for(int i=pos[l]+1;i<=pos[r]-1;i++){
			...
			int ps=upper_bound(d+left[i],d+right[i]+1,k)-d;
			res+=right[i]-ps+1;
		}
	} 
	return res;
