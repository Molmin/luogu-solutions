$\color{white}\text{网课摸鱼写题解}$

~~作为 ac 的第一道黑题当然要水一篇题解了（雾~~

没做过的建议先做 [SP703](https://www.luogu.com.cn/problem/solution/SP703)。本题与 [P7685](https://www.luogu.com.cn/problem/P7685) 总体思路乃至代码都~~基本一致（划掉~~。
...
$\color{white}\text{我的算法竞赛进阶指南呢？}$

>  在动态规划问题需要给出方案时，通常做法时而外使用一些与 DP 状态大小相同的数组记录下来每个状态的“最优解”是从何处转移而来的。最终，在 DP 求出最优解后，用过一次递归，沿着记录的每一步“转移来源”回到初态，即可得到一条从初态到最优解的转移路径，也就是所求的方案
...
设 $path$ 为 DP 在相应状态时存储下来的隐藏维度的服务员服务的前一个位置，但当那个服务员之前服务的位置是 $a_{i-1}$ 时，则说明那个隐藏维度的服务员的编号不变，所以将 $path$ 再改为 $0$ （便于后期回溯）。
$mx,my$ 初始值为答案所在数组的二三维。
...
unsigned short path[MAX_N][MAX_L][MAX_L];
