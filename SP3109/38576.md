给同学看了看自己随机跳到的这道题，然后他给我秒指原题...果然刷题多就是不一样。。。
...
int main(){ int op,x,y,l,r,mid,T=read();
	fp(kkksc03,1,T){
		tot=0,reads(s),n=s.length()-1,m=read();
...
双倍经验（单组数据！）：  [[JSOI2008]火星人](https://www.luogu.org/problemnew/show/P4036)
