打个广告：[blog](https://www.cnblogs.com/bztMinamoto/p/9428573.html)

没有LCT的题解来一发好了……因为根本不会楼下大佬的边分……

~~我已经想到了一个很好的LCT的解法可以这里空白太小写不下~~

讲真就是Qtree4的弱化版……具体的推导大家可以看看我Qree4的题解->[这里](https://minamoto.blog.luogu.org/solution-sp2666)（主要是懒得copy……）

然后就是一些比起之前需要注意的地方，首先维护的是最小，其次因为是每一个点到白点的最远距离，所以不需要维护路径了，只要维护虚链即可……

查询操作本来打算makeroot的……后来发现直接access+splay之后，点就已经在splay里深度最大了……直接输出rmn就行了……
```
//minamoto
#include<bits/stdc++.h>
