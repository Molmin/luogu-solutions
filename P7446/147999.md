[洛谷题面传送门](https://www.luogu.com.cn/problem/P7446)。

题意：

* 给定 $n-1$ 个数 $a_2, a_3,\ldots,a_n(a_i \in [1, i))$，一棵 $n$ 个节点的树是由所有 $a_i$ 与 $i$ 的连边。$q$ 次询问，有两种操作：
...
inline void work(){
	...
	rep(i, 1, p) update(i); while(m--){
		opt = read(); x = read() ^ lst; y = read() ^ lst;
		if(opt == 1) z = read() ^ lst, change(x, y, z);
		else printf("%d\n", lst = lca(x, y));
	}
}
