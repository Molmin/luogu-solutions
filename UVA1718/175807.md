[题目传送门](https://www.luogu.com.cn/problem/UVA1718)

[双倍经验传送门](https://www.luogu.com.cn/problem/P6913)
...
- 在网格图中，记 $g(p)$ 为满足下列条件的格点平行四边形数量：
	...
	2. 没有边竖直或水平；
	3. 面积为 $p$。
- 多测，每组测试数据有 $n$ 个询问，每个询问给出 $l,r$，问满足 $l\le p\le r$ 并使 $g(p)$ 最大的整数 $p$ 以及对应的 $g(p)$。
...
int main() {
	...
	for(int n; scanf("%d", &n)!=EOF; ) while(n--) {
		int x=read(), y=read(), d;
		if(x>y) d=x, x=y, y=d;
