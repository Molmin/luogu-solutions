...
$m$ 对链，每对链选择其中一个，使得选择的链不相交。

## 题解：

这边介绍一下更显然的一个做法。

可以发现是 2-sat 题目，建立两个点表示选 1 或者选 2 ，若两条链相交，则第一条链的选择连接第二条链的不选择。

考虑如何判断链交，可以扫描，在扫描到一条链的时候，使用树链剖分将这条链挂在线段树上，使用永久化标记即可。

扫描到下一条链的时候将链与线段树区间连接。

由于需要判断对于同一个颜色的蚂蚁不能有 选择1 -> 选择2，可以正反扫两遍处理。

边数 $mlog^2n$ ，时间复杂度也是这个。

（空间尽量大的开就好，只要不开爆，这边我开了748.52MB）
