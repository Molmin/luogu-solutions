...
得出，f[i]为调到每个格子时的最大得分， dmin=max(d-g,1)   dmax=d+g
...
#include<bits/stdc++.h>
using namespace std;
struct node{  //单调队列的元素x为和起点的距离 
  int x,v;    //              v为x距离格的最大得分 
} q[500005];  
int n,d,k;
int x[500005],s[500005],f[500005]; //x s数组为每格的距离和分值 
int check(int g){                  //f为状态数组，第i格的最大得分 
  ...
  memset(f,0,sizeof(f));
  for(int i=1;i<=n;i++){
    for(cur;(cur<i)&&(x[cur]<=x[i]-qmin);cur++){//把距离当前格qmin内的未添加到
                                                //单调队列的格子添加到单调队列 
      while((head<=tail)&&(q[tail].v<f[cur])) tail--; //值比要添加的小，寿命短的
                                                      //不会对以后的f有贡献，可
                                                      //以从队列中删除 
...
int main(){
  cin>>n>>d>>k;
  for(int i=1;i<=n;i++)
    cin>>x[i]>>s[i];     //输入数据已按x[i]从小到大排序，无需再排序 
  int l=0,r=x[n],ans=-1; //在0到x[n]范围内二分 
  int m=(l+r)/2;         //二分的标准程序 
  for(l;l<=r;m=(l+r)/2)
