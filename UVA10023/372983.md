...
[Link:UVA10023](https://www.luogu.com.cn/problem/UVA10023)

* **【关于本题】**

第一次抢到首$\color{green}\text A$，发篇题解纪念一下。

本题是[SP245](https://www.luogu.com.cn/problem/SP245)的双倍经验。

请注意，$\text{UVA}$ 的题是卡格式的。最后一行输出后，必须有且只有一个换行。很可惜，$\mathsf{\color{red}zltzlt}$ 因为没有特判最后一行输出，在最后输出了两个换行，从而没有抢到首$\color{green}\text A$。
...
每一组测试数据都只包含一个正整数 $Y(1\leq Y\leq 10^{1000})$。给定的数没有前导零，也没有多余的空白。保证 $\sqrt{Y}$ 为整数。
