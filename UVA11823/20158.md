...
这题并没有直接给出出发点以及终点，因此，任何一个点都有可能是出发点或者终点。题目说到路径不重合，因此需要对每条路径限流。而题目又说“share no intersections”，因此需要拆点限流。至此，思路已经很明了。

#### 步骤：
1. 拆点。每个点的入点向出点连接一条容量为1，费用为-1的边，保证这个点只被走一次。
2. 连边。容量为1，费用为0，保证这条边只被走一次
3. 连源汇点。源点->所有入点，所有出点->汇点，容量为1，费用为0
4. 两条路径。对于题目要求的两条路径，我们增加超级源，并向源点连一条容量为2，费用为0的边

#### 分析：
容量限制了走的次数，而费用记录了走过的点总数，因此，跑一边最小费用最大流，得到的最小费用的**相反数**便是答案。
